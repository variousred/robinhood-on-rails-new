...
class RobinhoodController < ApplicationController
  ...
  # before_action :set_oauth_token, except: [:login, :logout]
  def home
  ...
  def login
    Rails.logger.info("def login: Logging in")
    if params[:challenge_code].present?
      Rails.logger.info("Challenge code present")
    ...
    if response["mfa_required"] || response["accept_challenge_types"]
      Rails.logger.info("MFA or challenge required")
    ...
    Rails.logger.info("Response: #{response}")
    Rails.logger.info("MFA required: #{response["mfa_required"]}")
    Rails.logger.info("redirecting to: " + root_path(mfa_required: response["mfa_required"], challenge_required: response["accept_challenge_types"].present?))
  ...
  def positions_update
    # Fetch and process the positions data as in the positions controller action
    # ...
    @investments = {}
    @instruments = []
    get_positions
    ...
    @aggregate_positions.each do |aggregate_position|
      instrument = find_or_create_instrument aggregate_position["instrument"]
      @instruments << instrument
      aggregate_position["instrument"] = instrument
      @option_investments[instrument["symbol"]] = aggregate_position
    end
    ...
    respond_to do |format|
      Rails.logger.warn("positions_update")
      format.turbo_stream { render turbo_stream: turbo_stream.replace("positions", partial: "positions", locals: { investments: @investments, stock_lists: @stock_lists }) }
    end
  end
  ...
  def positions
    ...
    @stock_lists = current_user.update_stock_list :portfolio, @instruments
    Rails.logger.debug("stock_lists: #{@stock_lists.map(&:id)}")
    @stock_lists = @stock_lists.sort{|a,b| a.name.nil? ? 0 : 1}
    Rails.logger.debug("stock_lists: #{@stock_lists.map(&:id)}")

    
    respond_to do |format|
      Rails.logger.warn("format: #{format.inspect}")
      format.turbo_stream { render partial: "positions", content_type: "text/html", layout: false }
      format.html { render layout: false }
    end
  end
