export default function OrderForm(element) {
  console.log("OrderForm Function", element)
  ...
  var get_order_description = function(type) {
    if(side === "buy") {
      if(type == "Market"){
        ...
        return "Stop loss orders trigger a market order to buy when the stop price is reached. Enter a price higher than the current price."
    ...
    // SELL
      ...
      if(type == "Market"){
        ...
        return "Stop loss orders trigger a market order to sell when the stop price is met. Enter a price below the current price."
      } else if(type === "Stop limit") {
